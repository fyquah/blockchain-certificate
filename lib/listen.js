...
setInterval((function(){
    ...
    var update_block_count = function(cb){
        if (bitcoind.is_test_net()) {
            current_status_json["testnet"]["block_count"] += 1;
        } else {
            current_status_json["mainnet"]["block_count"] += 1;
        }
        console.log("config file is now " + JSON.stringify(current_status_json));
        fs.writeFile(config_file_path, JSON.stringify(current_status_json), "utf-8", "w" ,cb);
    };
    ...
    fs.readFile(config_file_path, "utf-8", "r", function(err, data){
        ...
        console.log(data);
        current_status_json = JSON.parse(data);
        console.log(current_status_json);
    });

    return function(){
        if(current_status_json === null || running_process === true) {
            return;
        }
        ...
        process.nextTick(function obtainBlock(){
            Q.promise(function(resolve, reject, notify){
                ...
                return Q.promise(function(resolve, reject, notify){
                    ...
                    system_block_count = current_status_json[(bitcoind.is_test_net() ? "testnet" : "mainnet")]["block_count"];
