...
var generate_database_error = function(err){
    if(err === null) {
        return null;
    } else {
        return {
            "code": constants.ERR_DATABASE_ERROR,
            "message": err
        };
    }
};
...
Model.prototype.create = function(callback){
    ...
    db.serialize(function(){
        db.run(sql_string , function(err, rows){
            if(err !== null) {
                err = generate_database_error(err);
            }

            if(typeof callback === "function") {
                callback.apply(that, [err, that]);
            }
        });
    });
};
    ...
    db.serialize(function(){
        db.all(sql_string, function(err, res){
            if(typeof callback == "function") {
                ...
                var entries = res.map(function(entry){
                    console.log(entry);
                    return new that(entry);
                });

                callback.apply(entries, [null, entries]);
            }
        });
    });
};

Model.find_by = function(obj, cb){
        ...
        db.all(sql_string, function(err, res){
            if (typeof cb == "function") {
                if(err !== null) {
                    ...
                    var entry = (res.length === 0 ? null : (new that(res[0])));
                    cb.apply(entry, [null, entry])
                }
            }
        });
    });
...
Model.query = function(sql_query, cb) {
    console.log("running : ");
    console.log(sql_query);
    var that = this;
    db.serialize(function(){
        db.all(sql_query, function(err, res){
            if (typeof cb == "function") {
                if(err !== null) {
                    cb(err);
                } else {
