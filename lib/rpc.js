...
var inspect_parameters_count  = function(count, params){
    ...
    if(typeof last === "function" && params.length === count + 1) {
        ...
        return { 
            "code": -1, 
            "message": "expected " + count + " parameter(s), but " + (arguments.length - 1) + " was given" 
        };
    }
};
var query_methods = {
...
var append_method = function(obj, type, method_name, arr){
    obj[method_name] = function(){
        ...
        console.log("valid request to run " + method_name + " is recevied");
        console.log("arguments : ");
        console.log(args);
        cb(null, args);
        // controller[method_name](args, function(err, res){
