...
var create_bitcoin_transaction = function(rights_output, receipients, values, cb){
    bitcoind("getnewadress", function(err, change_address){
        if (err) {
        ...
        bitcoind("listunspent", function(err, unspent_inputs){
            ...
            bitcoind("signrawtransaction " + tx.build().toHex(), function(err, stdout){
                var signed_transaction = stdout;
                bitcoind("sendrawtransaction " + signed_transaction, function(err, stdout){
                    cb(null, stdout);
                });
            });
        });
...
var broadcast_methods = {
    ...
    broadcastsignature: function(instruction){
        ...
        broadcast_wrapper(instruction.metadata, function(err, node, rights_output){
            ...
            bitcoind("getnewaddress", function(err, new_rights_output_addr){
